...
<template>
  <div class="app-container">
    <div class="filter">
      <div class="form-content">
        <el-form :model="filterForm">
          ...
          <el-form-item label="是否分配经销商">
            <el-select v-model="filterForm.hasDealer">
              <el-option
                v-for="(item, index) in hasDealerOptions"
                :key="index"
                :value="item.value"
                :label="item.label"
              ></el-option>
    ...
    >
      ...
      <el-table-column prop="hasDealer" label="是否分配经销商" width="125">
        <template v-slot="scope">
          <el-text :type="statusType[scope.row.hasDealer]">{{
            dealerMap[scope.row.hasDealer]
          }}</el-text>
        </template>
      </el-table-column>
      <el-table-column prop="dealerName" label="经销商名称" width="120" />
      ...
      <el-table-column prop="oper" label="操作" fixed="right" width="150">
        <template v-slot="scope">
          <el-button
            v-if="!scope.row.hasDealer"
            type="primary"
            size="small"
            @click="selectDealer(scope.row)"
            >分配经销商</el-button
          >
        </template>
      </el-table-column>
    ...
    <DealerDialog v-model="dealerDialog" :id="dealerId" @refresh="getData" />
  </div>
</template>

<script setup lang="ts">
import { saleCars } from "@/api/usedCar";
