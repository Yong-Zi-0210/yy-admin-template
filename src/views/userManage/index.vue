<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="levelTitle" label="会员等级" width="150" />
      <el-table-column prop="hasResume" label="是否可编辑简历" width="200">
        <template v-slot="scope">
          <span>{{ scope.row.hasResume ? "是" : "否" }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="hasSubmitResume"
        label="是否可直接投递简历"
        width="220"
      >
        <template v-slot="scope">
          <span>{{ scope.row.hasSubmitResume ? "是" : "否" }}</span>
        </template>
      </el-table-column>
